...
function Cart({ cart, items, dispatch }: { cart: any, items: any, dispatch: Function}) {
  ...
  const subTotal = cart.reduce((acc: any, item: any) => {
    const details = items.find((i: any) => i.id === item.id);
            ...
            <table>
              ...
              <tbody>
                {cart.map((item: any) => (
                  <tr key={item.id}>
                    <td>{item.quantity}</td>
                    <td>{items.find((i: any) => i.id === item.id).title}</td>
                    <td>
                      {`$${(
                        item.quantity
                        * items.find((i: any) => i.id === item.id).price
                      ).toFixed(2)}`}
                    </td>
                    <td>
                      <button
